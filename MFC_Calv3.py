...
# By Michael Haire
#Using SQLite to Store and Analyze the FLow and Voltage Data
...
def create_connection(r"/home/pi/db_MFC"):
 conn = None
 try:
  conn=sqlite3.connect(r"/home/pi/db_MFC")
...
def Volt():
  setp = adc.read_adc_difference(3, gain=GAIN)*0.0001875*2)
  feedb = adc.read_adc_difference(1, gain=GAIN)*0.0001875*2)
 return()
...
def Auto(FLOW):
 #Define Steps
 StepUp = 0
 Setpoint = 408+(204*StepUp) #DAC Voltage setting
 while StepUp < 19:
  dac.raw_value(int(Setpoint))
  time.sleep(30)
  Stime = int(time.time())
  Ctime = int(time.time())
  Dtime = int(time.time())
